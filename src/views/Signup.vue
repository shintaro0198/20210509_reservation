  ...
  <div id="signup">
    ...
    <v-col cols="9" xl="6" md="6" class="mt-16 mx-auto">
      <v-card>
        ...
        <v-card-text>
          <v-form>
            <v-text-field type="text" prepend-icon="mdi-account" label="ユーザーネーム" v-model="name">
            </v-text-field>
            <v-text-field type="email"
            prepend-icon="mdi-email" label="email" v-model="email">
            </v-text-field>
            <v-text-field :type="show? 'password': 'text'" prepend-icon="mdi-lock"
            :append-icon="show? 'mdi-eye':'mdi-eye-off'" 
            @click:append="show = !show" label="パスワード" v-model="password">
            </v-text-field>
          </v-form>
